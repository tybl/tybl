...
constexpr auto impl_lvalue_to_array(T (&a)[N], std::index_sequence<I...> /*indices*/)
    -> array<type_traits::remove_cv_t<T>, N> {
  return {{a[I]...}};
}

template <typename T, size_t N>
constexpr auto to_array(T (&a)[N]) noexcept(std::is_nothrow_constructible_v<T, T&>)
    -> array<type_traits::remove_cv_t<T>, N> {
...
template <typename T, size_t N, size_t... I>
constexpr auto impl_rvalue_to_array(T(&&a)[N], std::index_sequence<I...> /*indices*/)
    -> array<type_traits::remove_cv_t<T>, N> {
  return {{utility::move(a[I])...}};
}

template <typename T, size_t N>
constexpr auto to_array(T(&&a)[N]) noexcept(std::is_nothrow_move_constructible_v<T>)
    -> array<type_traits::remove_cv_t<T>, N> {
