...
class Parser {
  ...
  void parse_text(sentence& p_nmea, std::string p_s);    //fills the given NMEA sentence with the results of parsing the string.
...
public:
  ...
  void read_sentence(std::string p_cmd);        // called when parser receives a sentence from the byte stream. Can also be called by user to inject sentences.
