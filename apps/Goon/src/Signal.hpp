...
struct CollectorLast {
  ...
  CollectorResult result(void) { return mLast; }
...
private:
  ...
  struct SignalLink {
    ...
    ~SignalLink(void) { assert(mReferenceCount == 0); }
...
public:
  ...
  bool operator-=(size_t connection) { return mCallbackRing ? mCallbackRing->remove_sibling(connection) : false; }
...
static void bench_simple_signal() {
  ...
  printf("OK\n  Benchmark: Simple::Signal: %fns per emission (size=%zu): ",
         size_t(benchdone - benchstart) * 1.0 / size_t(i), sizeof(sig_increment));
}
...
uint64_t TestCounter::get(void) { return test_counter_var; }
