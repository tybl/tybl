...
struct Ledger::Impl {
  ...
  [[nodiscard]] std::vector<Account> accounts(std::string const& re) const {
    ...
    std::copy_if(m_accounts.begin(), m_accounts.end(), std::back_inserter(result),
                 [re](Account const& account) { return std::regex_search(account.name(), std::regex(re)); });
    return result;
  }
  ...
  [[nodiscard]] std::vector<Payee> payees(std::string const& re) const {
    ...
    std::copy_if(m_payees.begin(), m_payees.end(), std::back_inserter(result),
                 [re](Payee const& payee) { return std::regex_search(payee.name(), std::regex(re)); });
    return result;
  }
...
Ledger::Ledger()
  : m_pimpl(std::make_shared<Ledger::Impl>()) {}
  ...
  while (s) {
    switch (s.peek()) {
      ...
      case 'z':
        ...
        break;
      default: std::cerr << "default: Found a \"" << s.get() << "\"\n"; break;
    }
  } // while (s)
  return s;
}
