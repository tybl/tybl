...
struct ListNode {
  ...
  void set_free(bool free) { m_prev |= 1 & free; }

  bool get_free() const { return m_prev & 1; }

  identifier_t<TYPE>& next() { return m_next; }

  const identifier_t<TYPE>& next() const { return m_next; }
  ...
  const identifier_t<TYPE> get_prev() const { return identifier_t<TYPE>(m_prev >> 1); }
