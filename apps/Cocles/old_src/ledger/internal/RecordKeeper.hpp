...
struct RecordKeeper {
  ...
  TYPE& get_record(id_t id) { return at(id).data(); }

  const TYPE& get_record(id_t id) const { return at(id).data(); }

  bool is_empty() const { return (0 == used_count); }
  ...
  id_t get_first_free() const { return at(free_id).next(); }

  id_t get_first_used() const { return at(used_id).next(); }

  id_t get_next(id_t id) const { return at(id).next(); }

  id_t get_prev(id_t id) const { return at(id).prev(); }

  bool is_free(id_t id) const { return at(id).get_free(); }

  bool is_used(id_t id) const {
    return (1 < static_cast<uint64_t>(id)) && (static_cast<uint64_t>(id) < records.size()) && !at(id).get_free();
  }
...
private:
  ...
  node_t& at(identifier_t<TYPE> id) { return records.at(static_cast<uint64_t>(id)); }

  const node_t& at(identifier_t<TYPE> id) const { return records.at(static_cast<uint64_t>(id)); }
