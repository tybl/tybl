...
private:
  static constexpr bool index_cmp(const value_type& a, const value_type& b) { return a.index < b.index; }

  static constexpr bool value_cmp(const value_type& a, const value_type& b) { return a.value < b.value; }

  static constexpr bool lb_index_cmp(const value_type& obj, const std::size_t& index) { return obj.index < index; }

  static constexpr bool lb_value_cmp(const value_type& obj, const TYPE& value) { return obj.value < value; }
...
struct table_t {
  ...
  mapped_type& at(const key_type& key) { return m_entries.at(key); }

  const mapped_type& at(const key_type& key) const { return m_entries.at(key); }

  size_type count(const key_type& key) const { return m_entries.count(key); }
...
struct account_t {
  ...
  account_type_t type() const noexcept { return m_type; }
...
struct transaction_t {
  ...
  const date::year_month_day& date() const noexcept { return m_date; }

  const std::string& memo() const noexcept { return m_memo; }
...
struct adjustment_t {
  ...
  math::fixed_point_t<std::centi> amount() const { return m_amount; }
...
struct add_transaction_event_t {
  ...
  add_transaction_event_t(const std::string& input) { deserialize(input); }
  ...
  void deserialize(const std::string& input) {
    ...
    time =
        std::chrono::system_clock::time_point(std::chrono::microseconds(std::stoull(input.substr(start, end - start))));
...
struct add_account_event_t {
  ...
  add_account_event_t(const std::string& input) { deserialize(input); }
